...
async def start_router_step(
    ...
    """Показать пользователю шаг выбора роутера (Шаг 5/20)."""
...
async def select_router(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await query.message.reply_text(
        "📦 <b>Шаг 6/20: Количество роутеров</b>\n\n"
...
async def start_onu_step(update: Update, context: ContextTypes.DEFAULT_TYPE, db) -> int:
    ...
    if not onu_names:
        ...
        if chat_id:
            await context.bot.send_message(
                chat_id=chat_id,
                text="🔌 <b>Шаг 14/20: ONU Абонентский терминал</b>\n\n⏭️ Пропущено.",
                parse_mode="HTML"
            )
    ...
    text = (
        "🔌 <b>Шаг 14/20: ONU Абонентский терминал</b>\n\n"
...
async def start_media_step(update: Update, context: ContextTypes.DEFAULT_TYPE, db) -> int:
    ...
    if not media_names:
        ...
        if chat_id:
            await context.bot.send_message(
                chat_id=chat_id,
                text="🔄 <b>Шаг 15/20: Медиаконверторы</b>\n\n⏭️ Пропущено.",
                parse_mode="HTML"
            )

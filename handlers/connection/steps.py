...
async def new_connection_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    reply_markup = build_inline_keyboard([
        ...
        [InlineKeyboardButton("4️⃣ Дополнительная точка", callback_data='conn_type_additional')],
        [InlineKeyboardButton("5️⃣ Магистральная линия", callback_data='conn_type_magistral')]
    ])
...
4️⃣ Дополнительная точка - отдельная точка в существующем подключении
5️⃣ Магистральная линия - тип работ, включающий учет ВОЛС, крюков, ОРК и муфт
"""
...
async def select_connection_type(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if conn_type == 'magistral':
        return await start_magistral_from_type_selection(update, context)
...
async def contract_signed(update: Update, context: ContextTypes.DEFAULT_TYPE, db) -> int:
    """Обработка подтверждения договора и переход к шагу Телеграмм Бот"""
    query = update.callback_query
    await query.answer()

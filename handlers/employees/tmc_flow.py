...
STEP_INFO: Dict[str, tuple[int, str, str]] = {
    "fiber": (1, "ВОЛС", "🧵"),
    "twisted_external": (2, "Внеш. витая пара", "🪢"),
    "twisted_internal": (3, "Внут. витая пара", "🪢"),
    "router": (4, "Роутеры", "📡"),
    "snr": (5, "SNR оптические боксы", "🧰"),
    "onu": (6, "ONU абонентские терминалы", "🔌"),
    "media": (7, "Медиаконверторы", "🔄"),
    "sfp": (8, "SFP модули", "🧿"),
    "comment": (9, "Комментарий к выдаче", "📝"),
    "confirm": (10, "Подтверждение операции", "✅"),
}
...
def _default_tmc_data() -> Dict[str, object]:
    return {
        "employee_id": None,
        "fiber": 0.0,
        "twisted_external": 0.0,
        "twisted_internal": 0.0,
        "router": {"model": "-", "qty": 0},
        "snr": {"model": "-", "qty": 0},
        "onu": {"model": "-", "qty": 0},
        "media": {"model": "-", "qty": 0},
...
async def enter_fiber(flow: "EmployeeFlow", update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if text == SKIP_TEXT:
        ...
        return await _prompt_twisted_external_step(message, context)
...
async def enter_twisted(flow: "EmployeeFlow", update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Обработка шага с вводом внешней витой пары."""
    message = update.message
    text = (message.text or "").strip()
    ...
    if text == SKIP_TEXT:
        tmc_data["twisted_external"] = 0
        await message.reply_text(
            f"{_step_title('twisted_external')}\n\n⏭️ Пропущено.",
            parse_mode="HTML",
            reply_markup=ReplyKeyboardRemove(),
        )
        return await _prompt_twisted_internal_step(message, context)
    ...
    tmc_data["twisted_external"] = twisted_value
    await message.reply_text(
...
async def enter_twisted_internal(flow: "EmployeeFlow", update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Обработка шага с вводом внутренней витой пары."""
    message = update.message
    text = (message.text or "").strip()
    ...
    if text == SKIP_TEXT:
        tmc_data["twisted_internal"] = 0
        await message.reply_text(
            f"{_step_title('twisted_internal')}\n\n⏭️ Пропущено.",
            parse_mode="HTML",
            reply_markup=ReplyKeyboardRemove(),
        )
    ...
    try:
        ...
        await message.reply_text("⚠️ Введите корректное неотрицательное число (например, 120 или 45.5).")
        return TMC_ENTER_TWISTED_INTERNAL

    tmc_data["twisted_internal"] = twisted_value
    await message.reply_text(
        f"{_step_title('twisted_internal')}\n\n✅ Запланировано к выдаче: {twisted_value} м.",
        parse_mode="HTML",
        reply_markup=ReplyKeyboardRemove(),
...
async def confirm_operation(flow: "EmployeeFlow", update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if (
        tmc_data.get("fiber", 0) > 0
        or tmc_data.get("twisted_external", 0) > 0
        or tmc_data.get("twisted_internal", 0) > 0
    ):
        success = await run_in_thread(
            flow.db.add_material_to_employee,
            employee_id=employee_id,
            fiber_meters=tmc_data.get("fiber", 0),
            twisted_pair_external_meters=tmc_data.get("twisted_external", 0),
            twisted_pair_internal_meters=tmc_data.get("twisted_internal", 0),
...
async def _prompt_twisted_external_step(message, context: ContextTypes.DEFAULT_TYPE) -> int:
    employee = context.user_data.get("tmc_employee") or {}
    twisted_balance = employee.get("twisted_pair_external_balance", 0) or 0
    await message.reply_text(
        f"{_step_title('twisted_external')}\n\n"
...
async def _prompt_twisted_internal_step(message, context: ContextTypes.DEFAULT_TYPE) -> int:
    employee = context.user_data.get("tmc_employee") or {}
    twisted_balance = employee.get("twisted_pair_internal_balance", 0) or 0
    await message.reply_text(
        f"{_step_title('twisted_internal')}\n\n"
        f"📊 Текущий баланс: {twisted_balance} м\n"
...
async def _show_confirmation(message, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    summary = (
        ...
        f"🪢 Внеш. витая пара: {tmc_data.get('twisted_external', 0)} м\n"
        f"🪢 Внут. витая пара: {tmc_data.get('twisted_internal', 0)} м\n"
        f"🪢 Витая пара (общая): {(tmc_data.get('twisted_external', 0) or 0) + (tmc_data.get('twisted_internal', 0) or 0)} м\n"

...
| `handlers/employees/` | ~700+ | Крупный модуль, требует дальнейшей декомпозиции (`flow.py`, `tmc_flow.py`) |
...
- Продолжить декомпозицию `handlers/employees/` (в первую очередь `tmc_flow.py`)
...
| 3 | Поддерживать отсутствие legacy-дубликатов (выполнено для `handlers/employees.py`) | Низкая | 🟡 Средний |
...
| **3.4** | Контроль legacy | Периодически проверять отсутствие дублирующих legacy-файлов | 1-2 ч |

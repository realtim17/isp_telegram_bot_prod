...
class AccessManager:
    ...
    def __init__(self, db, base_ids: Optional[Iterable[int]] = None, admin_ids: Optional[Iterable[int]] = None) -> None:
        self.db = db
    ...
    def is_allowed(self, user_id: int) -> bool:
        ...
        with self._lock:
            return user_id in self.base_ids or user_id in self.admin_ids or user_id in self._db_ids
    ...
    def add_user(self, user_id: int, title: Optional[str], created_by: Optional[int]) -> (bool, Optional[str]):
        ...
        with self._lock:
            if user_id in self.base_ids or user_id in self.admin_ids or user_id in self._db_ids:
    ...
    def remove_user(self, user_id: int) -> (bool, Optional[str]):
        ...
        with self._lock:
            if user_id in self.base_ids or user_id in self.admin_ids:
